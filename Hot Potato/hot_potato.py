...
def configure(config):
    ...
    config.HotPotato.configure_setting("hotpot_admins", "What accounts can operate on ElAd? (One per line)")
...
strings_ita = {"already_in" : "%s sei già all'interno della partita di" + POTATO,
               ...
               "stop_timer" : CONTROL_BOLD + colors.RED + "STOP AL TEMPO!!! ",
               ...
               "quit_ok" : "Un giocatore ha lasciato la partita." + POTATO + "has to be started again.",
...
class PotatoGame:
    ...
    def explode_potato(self , bot , chan):
        ...
        time.sleep(1)
        bot.say(self.strings["kaboom_2"] % kaboom_player)
        time.sleep(1)
    ...
    def give(self , bot , giver , receiver , emergency = False):
        ...
        if receiver == self.players[giver]["giver"]:
            bot.say(self.strings["no_previous"] % (giver , receiver))
            return
        ...
        bot.say(self.strings["potato_received"][randint(0 , len(self.strings["potato_received"]))] % receiver)
...
class PotatoBot:
    ...
    def give(self , bot , trigger):
        ...
        self.games[trigger.sender].give(self , bot , trigger.nick ,receiver)
...
def stop(bot , trigger):
    if trigger.sender in game_chan and trigger.account in hotpot_admins:
        potato.stop(bot , trigger , forced=True)
        bot.say(f"{POTATO} : match forcefully STOPPED in {trigger.sender} by {trigger.nick}" , log_chan)
